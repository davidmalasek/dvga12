...
class AdjacencyList:
    ...
    def delete_node(self, name):
        ...
        if self.is_empty():
            return self.head()
        if name == self.name():
            return self.tail().delete_edges(name)
        self.cons(self.tail().delete_node(name))
        return self.head().delete_edges(name)
    ...
    def _add_edge(self, src, dst, weight):
        ...
        if src == self.name():
            ...
            return self.head()
        self.cons(self.tail()._add_edge(src, dst, weight))
        return self.head()
    ...
    def delete_edge(self, src, dst):
        ...
        if src == self.name():
            ...
            return self.head()
        self.cons(self.tail().delete_edge(src, dst))
        return self.head()
    ...
    def delete_edges(self, name):
        ...
        self.cons(self.tail().delete_edges(name))
        return self.head()
    ...
    def self_loops(self):
        ...
        if self.is_empty():
            return 0
        if self.edges().find(self.name()):
            count = 1
        else:
            count = 0
        return count + self.tail().self_loops()
    ...
    def adjacency_matrix(self):
        ...
        while not current.is_empty():
            source_index = node_to_index[current.name()]
            edge = current.edges()
            while not edge.is_empty():
                destination_index = node_to_index[edge.dst()]
                matrix[source_index][destination_index] = edge.weight()
                edge = edge.tail()
    ...
    def list_edges(self):
        ...
        if self.head().is_empty():
            return []
        return self.head().edges().list(self.head().name()) + self.tail().list_edges()
...
class Edge:
    ...
    def add(self, dst, weight=1):
        ...
        if dst < self.dst():
            ...
            new_edge.cons(self.head())
            return new_edge
        self.cons(self.tail().add(dst, weight))
        return self.head()
